...
module.exports = function (schema, options) {
  ...
  options.usernameUnique =
    options.usernameUnique === undefined ? true : options.usernameUnique;
  ...
  options.usernameCaseInsensitive = Boolean(
    options.usernameCaseInsensitive || false
  );
  ...
  schema.methods.setPassword = function (password, cb) {
    const promise = Promise.resolve()
      ...
      .then((hashRaw) => {
        this.set(
          options.hashField,
          Buffer.from(hashRaw, "binary").toString(options.encoding)
        );
      })
      .then(() => this);
    ...
    const promise = Promise.resolve().then(() => {
      ...
      return this.constructor
        ...
        .then((user) => {
          ...
          return {
            user: false,
            error: new errors.IncorrectUsernameError(
              options.errorMessages.IncorrectUsernameError
            ),
          };
        });
  ...
  schema.statics.findByUsername = function (username, opts, cb) {
    ...
    for (let i = 0; i < options.usernameQueryFields.length; i++) {
      const parameter = {};
      parameter[
        options.usernameQueryFields[i]
      ] = options.usernameCaseInsensitive
...
function pbkdf2Promisified(password, salt, options) {
  return new Promise((resolve, reject) =>
    pbkdf2(password, salt, options, (err, hashRaw) =>
...
function randomBytes(saltlen) {
  return new Promise((resolve, reject) =>
    crypto.randomBytes(saltlen, (err, saltBuffer) =>
